...
void choose_elevator(Passenger &p){
    ...
    for (; i < ELE_NUM; ++i){
        if(ele_vec[i].enabled_floor(p.dest_floor)){         //可以去到dest_floor，则加入集合
            en_ele.push_back(i);
...
//初始化乘客对象
Passenger::Passenger() : arrived(false), code(pas_code++), cur_floor(1), on_ele(false), finishd(false), chosen_ele(true) {
